...
export class EquationGeneratorService {
  ...
  getRandomIntNumber(min, max) {
    const maxVal = max + 1; // include max value in scope
    return Math.floor(Math.random() * (maxVal - min)) + min;
  }

  getAddEquation(spec: EquationSpec): Equation {
    const number1 = this.getRandomIntNumber(0, spec.maxFirstNumber);
    const number2 = this.getRandomIntNumber(0, spec.maxFirstNumber - number1);
    return {
      number1: number1,
      number2: number2,
  ...
  getMultEquation(spec: EquationSpec): Equation {
    const number1 = spec.strict
      ? spec.maxFirstNumber
      : this.getRandomIntNumber(0, spec.maxFirstNumber);
    const number2 = this.getRandomIntNumber(0, spec.maxSecondNumber);
